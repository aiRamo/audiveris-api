...
const parseXMLForCoordinates = (filePath) => {
    return new Promise((resolve, reject) => {
        fs.readFile(filePath, 'utf8', (err, data) => {
            ...
            const pageLayout = result['score-partwise']['defaults'][0]['page-layout'][0];
            parsedData.pageLayout.leftMargin = parseInt(pageLayout['page-margins'][0]['left-margin'][0], 10);
            parsedData.pageLayout.topMargin = parseInt(pageLayout['page-margins'][0]['top-margin'][0], 10);
            parsedData.pageLayout.pageHeight = parseInt(pageLayout['page-height'][0], 10);
            parsedData.pageLayout.pageWidth = parseInt(pageLayout['page-width'][0], 10);
            ...
            parts.forEach((part, partIndex) => {
                ...
                if (measure.print && measure.print[0]['system-layout']) {
                    measureData.leftMargin = parseInt(measure.print[0]['system-layout'][0]['system-margins'][0]['left-margin'][0], 10);
                }
    
                // Extract staff-distance
                if (measure.print && measure.print[0]['staff-layout'] && measure.print[0]['staff-layout'][0]['staff-distance']) {
                    measureData.staffDistance = parseInt(measure.print[0]['staff-layout'][0]['staff-distance'][0], 10);
                }
                ...
                if (measure.note) {
                    ...
                    if (note.stem && note.stem[0] && note.stem[0]['$'] && note.stem[0]['$']['default-y']) {
                        noteData.defaultY = parseInt(note.stem[0]['$']['default-y'], 10);
                    }
